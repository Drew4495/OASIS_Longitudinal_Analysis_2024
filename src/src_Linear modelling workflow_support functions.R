...
dotplot_num_highlight_subjects <- function(data, variables, highlight_subjects = list()) {
  
  data <- as.data.frame(data)
  
  # Create a 'color' column in 'data', defaulting to 'black'
  data$color <- "black"
  
  # Update 'color' column for highlighted subjects
  for (subject_ID in names(highlight_subjects)) {
    if (subject_ID %in% data$subject_ID) {
      data$color[data$subject_ID == subject_ID] <- highlight_subjects[subject_ID]
    } else {
      stop(paste("Subject ID", subject_ID, "not found in data."))
    }
  }
  
  variables <- na.omit(variables)
  if(length(variables) == 0) stop("ERROR: No variable has been specified. Skip this step if no NUMERIC variable is available.")
  if(sum(sapply(data[variables], FUN = is.numeric)) != length(variables)) stop("Incorrect assignment of variables. Specify NUMERIC variables only. Skip this step if none is available.")
  
  # Melt the data frame and include 'color' in the melted data
  dat_dotplot <- reshape2::melt(data, id.vars = c("subject_ID", "color"), measure.vars = variables)
  
  ggplot(dat_dotplot, aes(x = value, y = rep(seq(1, nrow(data), 1), length(variables)), color = color)) + 
    labs(x = "Value of variable", y = "Observation order in dataframe", title = "") +
    scale_y_continuous() + 
    scale_x_continuous() +
    geom_point() +  # 'color' aesthetic is already defined in ggplot()
    facet_wrap(~ variable, scales = "free", nrow = ceiling(log(length(variables)) + 0.1)) +
    scale_color_identity()  # Use actual color names in the 'color' column
}


## Histogram function Drew made to match dotplot function
histogram_num_drew <- function(data, variables, binwidths = NULL) {
  
  # Ensure input data is a dataframe
  data <- as.data.frame(data)
  
  # Remove any NA values from the variables vector
  variables <- na.omit(variables)
  
  # Check for presence of variables and their numeric nature
  if(length(variables) == 0) stop("ERROR: No variable has been specified. Skip this step if no NUMERIC variable is available.")
  if(sum(sapply(data[variables], is.numeric)) != length(variables)) {
    stop("Incorrect assignment of variables. Specify NUMERIC variables only. Skip this step if none is available.")
  }
  
  # Melt the data for plotting
  dat_histogram <- reshape2::melt(data, measure.vars = variables)
  
  # Start the ggplot
  p <- ggplot(dat_histogram, aes(x = value)) + 
    labs(x = "Value of variable", y = "Frequency") +
    facet_wrap(~ variable, scales = "free", nrow = ceiling(log(length(variables)) + 0.1))
  
  # Loop through each variable to add histogram with specific binwidth or default
  for (var in variables) {
    if (!is.null(binwidths) && !is.na(binwidths[var])) {
      bw <- binwidths[var]
      p <- p + geom_histogram(data = subset(dat_histogram, variable == var), binwidth = bw, fill = "blue", color = "black")
    } else {
      p <- p + geom_histogram(data = subset(dat_histogram, variable == var), fill = "blue", color = "black")
    }
  }
  
  # Apply the predefined theme modifications
  p <- p + plot0
...
barplot_fac <- function(data, variables) { 
  ...
  if(sum(sapply(data[variables], FUN = is.factor)) + sum(sapply(data[variables], FUN = is.character)) != length(variables)) {
    ...
    return(p)  # Return the ggplot object without printing it
  }
}
...
coll_num <- function(data, predictors) { 
  ...
  dat_plot$pair_key <- gsub(" against ", " \nagainst\n", dat_plot$pair_key)
...
## DREW: My edit fo the coll_num_fac function
coll_num_fac_drew <- function(data, predictors, title_font_size = 12, x_axis_font_size = 10, y_axis_font_size = 10) {
  ...
  if(sum(sapply(data[predictors], FUN = is.numeric)) < 1) {
    ...
    if(sum(sapply(data[predictors], FUN = is.factor)) < 1) {
      ...
      dat_plot <- data.frame(expand.grid.unique(num, fac))
      colnames(dat_plot) <- c("value", "levels")
      dat_plot <- cbind(data.frame(pair_key = paste(dat_plot[[1]], dat_plot[[2]], sep = " against "), stringsAsFactors = FALSE), dat_plot)
      
      
      dat_plot <- cdata::rowrecs_to_blocks(data, dat_plot)
      
      plot <- lapply(levels(as.factor(dat_plot$pair_key)), function(pair_key_value) {
        ggplot(subset(dat_plot, pair_key == pair_key_value), aes(x = levels, y = value)) +
          geom_boxplot(outlier.shape = NA) +
          geom_quasirandom(method = "smiley") +
          labs(y = sub(' against.*', '', pair_key_value), x = sub('.*against ', '', pair_key_value)) + #, title = pair_key_value) +
          theme(plot.title = element_text(size = title_font_size),
                axis.title.x = element_text(size = x_axis_font_size),
                axis.title.y = element_text(size = y_axis_font_size))
      })
...
## DREW: My edit to the coll_fac function
coll_fac_drew <- function(data, predictors, title_font_size = 12, x_axis_font_size = 10, y_axis_font_size = 10) {
  ...
  if(sum(sapply(data[predictors], FUN = is.factor)) != length(predictors)) {
    ...
    plot <- lapply(levels(as.factor(dat_plot$pair_key)), function(pair_key_value) {
      ggplot(subset(dat_plot, pair_key == pair_key_value)) +
        ...
        labs(y = sub(' against.*', '', pair_key_value), 
             x = sub('.*against ', '', pair_key_value)) +  # Removed title from each subplot
        theme(
          axis.title.x = element_text(size = x_axis_font_size),
          axis.title.y = element_text(size = y_axis_font_size),
          plot.title = element_text(size = title_font_size)  # Assuming you might want to add a main title later
        )
    })
...
relat_single_drew <- function(data, response, predictors, y.limits = NULL, y.breaks = NULL, x_axis_font_size = 12, y_axis_font_size = 12, save_file_path = NULL) {
  ...
  if(is.null(y.limits)) {
    y.limits <- c(min(data[,response], na.rm = TRUE), max(data[,response], na.rm = TRUE))
  } 
  ...
  if(length(num) >= 1) {
    plots <- c(plots, lapply(num, function(num_var) {
      ggplot(data, aes_string(x = num_var, y = response)) + 
        geom_point() +
        labs(y = response, x = num_var) +
        theme(axis.title.x = element_text(size = x_axis_font_size), 
              axis.title.y = element_text(size = y_axis_font_size))
    }))
  }
  
  # Handling factor predictors
  if(length(fac) >= 1) {
    plots <- c(plots, lapply(fac, function(fac_var) {
      ggplot(data, aes_string(x = fac_var, y = response)) + 
        geom_boxplot() +
        labs(y = response, x = fac_var) +
        theme(axis.title.x = element_text(size = x_axis_font_size), 
              axis.title.y = element_text(size = y_axis_font_size))
    }))
  }
  ...
  # Saving plots
  if(length(plots) > 16) {
    mp <- marrangeGrob(grobs = plots, ncol = 4, nrow = 4, top = "Combined Plots")
    ggsave(save_file_path, mp, height = 20, width = 30, units = "cm")
  } else {
    combined_plot <- do.call(grid.arrange, c(plots, ncol = 4))
    ggsave(save_file_path, combined_plot, height = 20, width = 30, units = "cm")
  }
}
...
residual_plots <- function(data, modelTMB, response) {
  ...
  #do.call("grid.arrange", c(plot, nrow = ceiling(log(length(plot) + 0.1))))
  plot_grob <- do.call("arrangeGrob", c(plot, nrow = ceiling(log(length(plot) + 0.1))))
...
residual_plots_predictors <- function(data, modelTMB, predictors, y_axis_font_size=12) {
...
if(length(num) > 1) {
  plot1 <- lapply(levels(as.factor(dat_plot$num)), function(num_value) {
    ggplot(subset(dat_plot, num == num_value), aes(x = value, y = mod.res)) + 
      ...
      theme(aspect.ratio = 1) +
      plot0 +
      theme(axis.title.y = element_text(size = y_axis_font_size))
    })
  } else {
  ...
  if(length(num) == 1) {
    plot1 <- list(ggplot(data, aes(x = !!sym(num), y = mod.res)) + 
                    ...
                    labs(y = "Standardised residuals", 
                         x = num, 
                         title = "") +
                    theme(aspect.ratio = 1, axis.title.y = element_text(size = y_axis_font_size)) +
                    plot0 +
                    theme(axis.title.y = element_text(size = y_axis_font_size))
                  )
    }  else {plot1 <- NULL}}
...
residual_plots_interactions <- function(data, modelTMB, predictors, savepath) { 
  ...
  if(length(na.omit(predictors)) < 2) {stop("ERROR: At least two predictors are required")} else {
    ...
    if(length(num) > 1) {
      ...
      plot1 <- lapply(levels(dat_plot$pair_key), function(pair_key_value)  {
        ...
        group_sizes <- table(sub$x)
        
        # Print the group sizes
        print(paste("Group sizes for pair_key_value:", pair_key_value))
        print(group_sizes)
        
        
        ggplot(data = sub, aes(x = y, y = mod.res, color = x, group = x)) + 
          scale_y_continuous(limits = c(0,1)) +
          geom_smooth(method = "loess", span = 3, se = T) + 
    ...
    if(length(plot) > 9) {
      mp <-  marrangeGrob(grobs = plot, 
                          ncol = 2, 
                          nrow = 2)
      ggsave(savepath, mp, width = 12, height = 12, units = "in")} else {
...
residual_plots_random_slopes_drew <- function(data, modelTMB, fixed_eff, random_eff, savepath, custom_theme = list()) {
  
  data <- as.data.frame(data)
  
  if(length(ranef(modelTMB)$cond) < 1) {
    stop("ERROR: Your model does not contain random factors, skip this step.")
  } else {
    if(length(na.omit(fixed_eff)) < 1 || length(na.omit(random_eff)) < 1) {
      stop("ERROR: Please enter at least one predictor and random factor.")
    } else {
      mod.res <- simulateResiduals(fittedModel = modelTMB)$scaledResiduals  # from DHARMa
      ...
      # Start the PDF device, all plots will be saved to this file
      pdf(file = savepath, width = 8, height = 11)
      
      lapply(random_eff, function(random_eff_value) {
        if(length(num) > 0) {
          lapply(num, function(x_value) {
            plot <- ggplot(data, aes_string(x = x_value, y = "mod.res")) + 
              scale_y_continuous(limits = c(0,1)) +
              geom_point() +
              geom_smooth(se = F, method = "glm") +  # or loess?
              facet_wrap(as.formula(paste("~", random_eff_value))) + 
              labs(y = "Standardised residuals", x = x_value, 
                   title = paste("Std residuals as a function of", x_value, "split by", random_eff_value)) +
              theme(panel.spacing.x = unit(1, "lines"))
            
            # Apply custom theme modifications
            for (theme_element in custom_theme) {
              plot <- plot + theme_element
            }
            
            print(plot)  # Explicitly print the plot to render it in the PDF
          })
        }
        
        if(length(fac) > 0) {
          lapply(fac, function(x_value) {
            plot <- ggplot(data, aes_string(x = x_value, y = "mod.res")) + 
              scale_y_continuous(limits = c(0,1)) +
              geom_boxplot(outlier.shape = NA) +
              geom_quasirandom(method = "smiley", alpha = 0.6, col = "grey40", cex = 0.7) +
